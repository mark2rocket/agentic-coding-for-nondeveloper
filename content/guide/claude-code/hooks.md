...
<Callout type="success" title="가장 확실한 방법">

클로드 코드의 설정 파일인 `settings.json`에 훅을 추가하는 것이 가장 확실한 방법입니다.

</Callout>

**방법 1: 설정 파일(settings.json) 직접 수정하기**

<LevelBox>

**1단계: 설정 파일 찾기**
- 프로젝트 폴더 내의 `.claude/settings.json` 파일을 엽니다 (없으면 생성)
- 또는 전역 설정(`~/.claude/config.json`)을 수정할 수도 있습니다
...
```json
{
  "hooks": {
    "Stop": [
      {
        "type": "command",
        "command": "say '주인님, 요청하신 작업이 완료되었습니다.'"
      }
    ]
  }
}
...
**코드 설명:**
- `"Stop"`: 클로드 코드가 응답 생성을 마치거나 작업을 중단했을 때 발동하는 이벤트
- `"command": "say ..."`: macOS의 경우 `say` 명령어로 텍스트를 음성으로 읽어줍니다
- Windows라면 `echo ^G` (비프음) 같은 다른 명령어를 사용하거나, 별도의 알림 스크립트 필요

</LevelBox>

**방법 2: 자연어로 요청하기 (가장 쉬운 방법)**

<LevelBox>

직접 코드를 수정하기 어렵다면, 클로드 코드에게 자연어로 명령하여 훅을 설정하게 시킬 수 있습니다.

**프롬프트 입력 예시:**

```
작업이 완료되면 음성으로 "주인님, 작업 완료되었습니다"라고
알려주는 훅을 설정해줘. settings.json에 Stop 이벤트 훅으로 추가해줘.
```

이렇게 입력하면 클로드 코드가 알아서 `settings.json` 파일을 찾아 해당 코드를 작성해 줍니다.
...
**작동 방식:**
- 설정이 완료되면, 클로드 코드가 긴 코딩 작업을 마친 후 대기 상태로 돌아갈 때 자동으로 스피커를 통해 설정한 멘트가 나옵니다.

**실무 효과:**
- "개발동생님 요청하신 작업이 완료되었습니다"처럼 음성 알림을 설정해 두면, 작업을 시켜놓고 다른 업무를 보거나 자리를 비워도 소리를 듣고 바로 복귀할 수 있어 효율적입니다.

**응용 가능:**
- 소리뿐만 아니라 시스템 알림(Notification)을 띄우거나
- 슬랙(Slack)으로 메시지를 보내는 등 다양한 방식으로 응용 가능
